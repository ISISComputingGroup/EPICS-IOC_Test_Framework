...
def get_beamline(macros):
    ...
    add_parameter(
        AxisParameter("out_pos", out_comp, ChangeAxis.POSITION, autosave=False), modes=[nr]
    )
    add_driver(
        IocDriver(
            out_comp,
    ...
    out_comp_auto = add_component(
        Component("out_comp_auto", PositionAndAngle(0.0, 1.5 * SPACING, 90))
    )
    ...
    theta_for_init = add_component(
        ThetaComponent("theta_init_comp", PositionAndAngle(0.0, 3 * SPACING, 90))
    )
    add_parameter(
        AxisParameter("theta_auto", theta_for_init, ChangeAxis.ANGLE, autosave=True), modes=[nr]
    )

    det_for_init = add_component(
        TiltingComponent("det_init_comp", PositionAndAngle(0.0, 4 * SPACING, 90))
    )
    add_parameter(
        AxisParameter("init", det_for_init, ChangeAxis.POSITION, autosave=False), modes=[nr]
    )
    add_driver(IocDriver(det_for_init, ChangeAxis.POSITION, MotorPVWrapper("MOT:MTR0103")))
    theta_for_init.add_angle_to(det_for_init)

    det_for_init_auto = add_component(
        TiltingComponent("det_init_auto_comp", PositionAndAngle(0.0, 5 * SPACING, 90))
    )
    add_parameter(
        AxisParameter("init_auto", det_for_init_auto, ChangeAxis.POSITION, autosave=True),
        modes=[nr],
    )

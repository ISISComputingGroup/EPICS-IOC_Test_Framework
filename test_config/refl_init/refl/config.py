...
def get_beamline():
    ...
    sm_comp = ReflectingComponent("sm", PositionAndAngle(0.0, 3*SPACING, 90))
    det_for_init = TiltingComponent("det_init_comp", PositionAndAngle(0.0, 5*SPACING, 90))
    det_for_init_auto = TiltingComponent("det_init_auto_comp", PositionAndAngle(0.0, 6*SPACING, 90))
    theta_for_init = ThetaComponent("theta_init_comp", PositionAndAngle(0.0, 4*SPACING, 90), [det_for_init])

    comps = [sm_comp, out_comp, in_comp, theta_for_init, det_for_init]
    ...
    sm_angle = AngleParameter("sm_angle", sm_comp, autosave=False)
    ...
    params = [is_out, out_pos, is_in, in_pos, sm_angle, theta_auto, init, init_auto]
    ...
    drivers = [DisplacementDriver(out_comp, MotorPVWrapper("MOT:MTR0101"), INIT_OUT_POSITION, tolerance_on_out_of_beam_position=0.5),
               ...
               AngleDriver(sm_comp, MotorPVWrapper("MOT:MTR0103")),
               DisplacementDriver(det_for_init, MotorPVWrapper("MOT:MTR0104")),
               DisplacementDriver(det_for_init_auto, MotorPVWrapper("MOT:MTR0105")), ]

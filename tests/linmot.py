...
class LinmotTests(unittest.TestCase):
    ...
    @parameterized.expand(
        [
            ("Low limit", MTR_LOW_LIMIT_DEFAULT),
            ("Normal value", 12.56),
            ("High limit", MTR_HIGH_LIMIT_DEFAULT),
        ]
    )
    def test_GIVEN_motor_destination_WHEN_motor_given_destination_THEN_move_to_correct_place(
        self, _, target_position
    ):
    ...
    def test_GIVEN_new_acceleration_WHEN_set_acceleration_THEN_acceleration_set(
        self, _, target_acceleration
    ):
    ...
    def test_GIVEN_new_position_WHEN_moving_THEN_DMOVE_status_updated(self):
        expected_value = 0
        self.ca_linmot.set_pv_value(
            MTR_VELOCITY, 0.01
        )  # Slow axis so it cant reach target before stop sent
    ...
    def test_GIVEN_using_motion_set_points_and_at_non_zero_position_WHEN_ioc_restarted_THEN_emulator_does_not_move(
        self,
    ):
        target_position = 10
        self.ca_linmot.set_pv_value(MTR_SETPOINT, target_position)

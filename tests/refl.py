...
IOCS = [
    # Deliberately start the REFL server first to check on waiting for motors functionality
    {
        ...
        "macros": {"CONFIG_FILE": "config_init.py"},
        "environment_vars": {
...
class ReflTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.ca_galil = ChannelAccess(
            default_timeout=30, device_prefix="MOT", default_wait_time=0.0
        )
    ...
    def set_up_velocity_tests(self, velocity):
        ...
        self.ca_galil.set_pv_value(
            "MTR0105.VELO", FAST_VELOCITY
        )  # Remove angle as a speed limiting factor
    ...
    def test_GIVEN_jaws_set_to_value_WHEN_change_sp_at_low_level_THEN_jaws_sp_rbv_does_not_change(
        ...
        self.ca.assert_that_pv_is_number(
            "PARAM:S1HG", expected_gap_in_refl, timeout=15, tolerance=MOTOR_TOLERANCE
        )

        self.ca_galil.set_pv_value("JAWS1:HGAP:SP", expected_change_to_gap)
        self.ca_galil.assert_that_pv_is_number(
            "JAWS1:HGAP", expected_change_to_gap, timeout=15, tolerance=MOTOR_TOLERANCE
        )
    ...
    def test_GIVEN_engineering_correction_WHEN_move_THEN_move_includes_engineering_correction(self):
        ...
        self.ca.assert_that_pv_is(
            "COR:MOT:MTR0206:DESC", "Interpolated from file s4_correction.dat on MOT:MTR0206 for s5"
        )
        self.ca.assert_that_pv_is(
            "COR:MOT:MTR0206", theta / 10.0
        )  # s4 correction is a 1/10 of theta
        ...
        self.ca_galil.assert_that_pv_is_number(
            "MTR0206.RBV", expected_position, tolerance=MOTOR_TOLERANCE, timeout=30
        )
    ...
    def test_GIVEN_jaws_not_at_zero_WHEN_define_motor_position_for_jaw_gaps_THEN_jaws_position_are_changed_without_move(
        ...
        self.ca.assert_setting_setpoint_sets_readback(
            initial_gap, "PARAM:S1VG", expected_alarm=None, timeout=30
        )
        self.ca.assert_setting_setpoint_sets_readback(
    ...
    def test_GIVEN_jaws_not_at_zero_WHEN_define_motor_position_for_jaw_centres_THEN_jaws_position_are_changed_without_move(
        ...
        self.ca.assert_setting_setpoint_sets_readback(
            initial_gap, "PARAM:S1HG", expected_alarm=None, timeout=30
        )
        self.ca.assert_setting_setpoint_sets_readback(
    ...
    def test_GIVEN_parameter_not_in_manager_mode_WHEN_define_position_using_no_action_do_action_THEN_position_is_not_defined(
        self,
    ):
        new_position = 10
    ...
    def test_GIVEN_component_with_multiple_parked_positions_WHEN_moving_out_of_beam_THEN_driver_moves_to_correct_out_of_beam_position_based_on_beam_interception(
        ...
        self.ca_galil.assert_that_pv_is_number(
            "MTR0102.VAL", expected_out_of_beam_position, timeout=5
        )
    ...
    def test_GIVEN_displacement_parameter_WHEN_sm_angle_changed_THEN_parameters_reflect_displacement_of_component(
        self,
    ):
        ...
        self.ca.assert_that_pv_is_number(
            "PARAM:DISP_POS", 1, tolerance=MOTOR_TOLERANCE
        )  # should equal distance to SM

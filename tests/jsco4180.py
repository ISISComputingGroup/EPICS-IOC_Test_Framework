...
class Jsco4180Tests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.ca = ChannelAccess(
            device_prefix=DEVICE_PREFIX, default_timeout=30, default_wait_time=0.0
        )
    ...
    def test_GIVEN_wrong_component_on_device_WHEN_send_new_sp_THEN_state_machine_aborts_resend(
        self,
    ):
        value = 50
    ...
    def test_GIVEN_an_ioc_WHEN_set_flowrate_THEN_flowrate_setpoint_is_correct(self):
        ...
        sleep(
            2 * error_delay
        )  # To make sure we're not in the middle of the error-checking state machine
    ...
    def test_GIVEN_an_ioc_WHEN_set_flowrate_and_pump_volume_THEN_ioc_uses_rbv_for_calculation_of_remaining_time(
        self,
    ):
    ...
    def test_GIVEN_an_ioc_WHEN_set_flowrate_and_pump_time_THEN_ioc_uses_rbv_for_calculation_of_remaining_volume(
        self,
    ):
        expected_sp_value = 1.000
        expected_rbv_value = 2.000
    ...
    def test_GIVEN_an_ioc_WHEN_set_maximum_pressure_limit_THEN_maximum_pressure_limit_is_correct(
        self,
    ):
        expected_value = 200
        self.ca.assert_setting_setpoint_sets_readback(expected_value, "PRESSURE:MAX")

    def test_GIVEN_an_ioc_WHEN_set_minimum_pressure_limit_THEN_minimum_pressure_limit_is_correct(
        self,
    ):
        expected_value = 100
    ...
    def test_GIVEN_command_seq_that_would_crash_pump_WHEN_command_seq_called_THEN_pump_crashes(
        self,
    ):
    ...
    def test_GIVEN_pump_running_WHEN_set_file_number_command_called_THEN_program_is_busy_error(
        self,
    ):
    ...
    @parameterized.expand(
        [
            ("low_set_time", 100, 1, 1),
            ("high_set_time", 1000, 10, 1),
            ("non_standard_set_time", 456, 5, 1),
        ]
    )
    @unstable_test(max_retries=5)
    def test_GIVEN_pump_for_volume_WHEN_pumping_THEN_device_is_pumping_set_volume(
        self, _, time, volume, flowrate
    ):
        ...
        expected_time = (
            set_volume * set_flowrate * 60
        )  # flow rate units = mL/min, so convert to seconds

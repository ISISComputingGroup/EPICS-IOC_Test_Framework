...
IOCS = [
    {
        ...
        "macros": {
            "NUM_USER_VARS": NUM_USER_VARS,
            "NUM_USER_BUTTONS": NUM_USER_BUTTONS
        },
    },
...
class InstEtcTests(unittest.TestCase):
    ...
    @parameterized.expand([("I", 12),
                           ("R", 12.34),
                           ("S", "123abc")])
    def test_GIVEN_loaded_WHEN_set_and_read_user_integer_THEN_integer_is_set(self, test_type, value):
        for var_index in range(NUM_USER_VARS + 1):
            self.ca.assert_setting_setpoint_sets_readback(value, readback_pv="USER:{}{}".format(test_type, var_index))

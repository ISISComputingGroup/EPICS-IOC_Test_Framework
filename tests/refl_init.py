...
class ReflTests(unittest.TestCase):
    ...
    def test_GIVEN_theta_init_to_non_zero_and_det_pos_not_autosaved_WHEN_initialising_det_pos_THEN_det_pos_sp_is_initialised_to_rbv_minus_offset_from_theta(
        ...
        expected_value = (
            DET_INIT_POS - SPACING
        )  # angle between theta component and detector is 45 deg

        self.ca.assert_that_pv_is_number("PARAM:INIT:SP:RBV", expected_value)

    def test_GIVEN_theta_is_non_zero_and_param_is_autosaved_WHEN_initialising_detector_height_param_THEN_param_sp_is_initialised_to_autosave_value(
        self,
    ):
    ...
    def test_GIVEN_component_out_of_beam_WHEN_starting_up_ioc_THEN_inbeam_sp_false_and_pos_sp_zero(
        self,
    ):
        expected_inbeam = "OUT"
        expected_pos = 0.0
    ...
    def test_GIVEN_component_in_beam_WHEN_starting_up_ioc_THEN_inbeam_sp_true_and_pos_sp_accurate(
        self,
    ):
        expected_inbeam = "IN"
    ...
    def test_GIVEN_motor_values_set_WHEN_starting_refl_ioc_THEN_parameter_rbvs_are_initialised_correctly(
        self,
    ):
    ...
    def test_GIVEN_optional_macro_is_set_to_true_THEN_true_value_passed_into_reflectometry_config(
        self,
    ):
        # See macro values in IOC dict above
        self.ca.assert_that_pv_exists("CONST:OPTIONAL_1")

    def test_GIVEN_optional_macro_is_set_to_false_THEN_false_value_passed_into_reflectometry_config(
        self,
    ):
        # See macro values in IOC dict above

...
class Lakeshore460Tests(unittest.TestCase):
    ...
    def test_GIVEN_unit_set_gauss_WHEN_read_THEN_unit_is_gauss(self):
        ...
        self.ca.set_pv_value("UNIT:SP", unit_flag)
        self.ca.assert
        self.ca.assert_that_pv_is("UNIT", unit_string)
    ...
    def test_GIVEN_source_set_WHEN_read_THEN_source_is_set_value(self):
        for key in vector_dict:
            ...
            self.ca.set_pv_value("SOURCE:SP", set_value)
            self.ca.assert_that_pv_is("SOURCE", expected_value)
    ...
    def test_GIVEN_output_mode_set_DC_WHEN_read_THEN_output_mode_is_dc(self):
        for chan in channels_array:
            ...
            self.ca.set_pv_value("{}:MODE:SP".format(chan), mode_set)
            self.ca.assert_that_pv_is("{}:MODE".format(chan), expected_mode)
    ...
    def test_GIVEN_output_mode_set_AC_WHEN_read_THEN_output_mode_is_ac(self):
        for chan in channels_array:
            ...
            self.ca.set_pv_value("{}:MODE:SP".format(chan), mode_set)
            self.ca.assert_that_pv_is("{}:MODE:SP".format(chan), expected_mode)

    def test_GIVEN_prms_set_rms_WHEN_read_THEN_prms_is_rms(self):
        for chan in channels_array:
            set_prms = UnitFlags.PEAK
            expected_prms = UnitStrings.PEAK
            self.ca.set_pv_value("{}:PRMS:SP".format(chan), set_prms)
            self.ca.assert_that_pv_is("{}:PRMS".format(chan), expected_prms)
    ...
    def test_GIVEN_display_filter_set_on_WHEN_read_THEN_display_filter_is_set_value(self):
        for chan in channels_array:
            ...
            self.ca.set_pv_value("{}:FILTER:SP".format(chan), set_filter_status)
            self.ca.assert_that_pv_is("{}:FILTER".format(chan), expected_filter_status)
    ...
    def test_GIVEN_rel_mode_status_set_on_WHEN_read_THEN_rel_mode_status_is_set_value(self):
        for chan in channels_array:
            ...
            self.ca.set_pv_value("{}:RELMODE:SP".format(chan), set_rel_mode_status)
            self.ca.assert_that_pv_is("{}:RELMODE".format(chan), expected_rel_mode_status)
    ...
    def test_GIVEN_auto_mode_status_set_on_WHEN_read_THEN_auto_mode_status_is_set_value(self):
        for chan in channels_array:
            ...
            self.ca.set_pv_value("{}:AUTO:SP".format(chan), set_auto_mode_status)
            self.ca.assert_that_pv_is("{}:AUTO".format(chan), expected_auto_mode_status)
    ...
    def test_GIVEN_max_hold_status_set_on_WHEN_read_THEN_max_hold_status_is_set_value(self):
        for chan in channels_array:
            ...
            self.ca.set_pv_value("{}:MAXHOLD:SP".format(chan), set_max_hold_status)
            self.ca.assert_that_pv_is("{}:MAXHOLD".format(chan), expected_max_hold_status)
    ...
    def test_GIVEN_channel_status_set_on_WHEN_read_THEN_channel_status_is_set_value(self):
        for chan in channels_array:
            ...
            self.ca.set_pv_value("{}:STATUS:SP".format(chan), set_channel_status)
            self.ca.assert_that_pv_is("{}:STATUS".format(chan), expected_channel_status)
    ...
    def test_GIVEN_filter_windows_set_WHEN_read_THEN_alarm_is_major(self):
        for chan in channels_array:
            filter_windows = 11
            self.ca.set_pv_value("{}:FWIN:SP".format(chan), filter_windows)
            self.ca.assert_pv_alarm_is("{}:FWIN".format(chan), self.ca.ALARM_MAJOR)
            self.ca.assert_that_pv_is("{}:FWIN".format(chan), filter_windows)

    def test_GIVEN_filter_windows_set_WHEN_read_THEN_alarm_is_none(self):
        for chan in channels_array:
            filter_windows = 4
            self.ca.set_pv_value("{}:FWIN:SP".format(chan), filter_windows)
            self.ca.assert_pv_alarm_is("{}:FWIN".format(chan), self.ca.ALARM_NONE)
            self.ca.assert_that_pv_is("{}:FWIN".format(chan), filter_windows)
    
    def test_GIVEN_filter_points_set_WHEN_read_THEN_alarm_is_major(self):
        for chan in channels_array:
            filter_points = 65
            self.ca.set_pv_value("{}:FNUM:SP".format(chan), filter_points)
            self.ca.assert_pv_alarm_is("{}:FNUM".format(chan), self.ca.ALARM_MAJOR)
            self.ca.assert_that_pv_is("{}:FNUM".format(chan), filter_points)

    def test_GIVEN_filter_points_set_WHEN_read_THEN_alarm_is_none(self):
        for chan in channels_array:
            filter_points = 10
            self.ca.set_pv_value("{}:FNUM:SP".format(chan), filter_points)
            self.ca.assert_pv_alarm_is("{}:FNUM".format(chan), self.ca.ALARM_NONE)
            self.ca.assert_that_pv_is("{}:FNUM".format(chan), filter_points)
    ...
    def test_GIVEN_range_set_WHEN_read_THEN_range_is_set_value(self):
        for chan in channels_array:
            for key in range_dict:
                ...
                self.ca.set_pv_value("{}:RANGE:SP".format(chan), set_range)
                self.ca.assert_that_pv_is("{}:RANGE".format(chan), expected_range)

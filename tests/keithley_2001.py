...
def _reset_readings(ca):
    ...
    ca.assert_setting_setpoint_sets_readback(
        ["0"] * 20, pv, set_point_pv=pv, expected_value="".join(["0"] * 20)
    )
...
def _setup_channel_to_test(ca, lewis, channel, value=None):
    ...
    if value is not None:
        lewis.backdoor_run_function_on_device(
...
class ScanningSetupTests(unittest.TestCase):
    ...
    @parameterized.expand(parameterized_list([[1, 2], [1, 2, 3, 4], [6, 7, 8, 9], [1, 5, 10]]))
...
class ErrorTests(unittest.TestCase):
    def _simulate_error(self, error_code, error_message):
        if IOCRegister.uses_rec_sim:
            ...
            self._lewis.backdoor_run_function_on_device(
                "set_error_via_the_backdoor", [error_code, error_message]
            )
    ...
    def test_that_GIVEN_a_device_not_scanning_on_any_channels_with_no_error_THEN_the_IOC_reads_that_there_are_no_errors(
        ...
        self.ca.assert_that_pv_is(
            "ERROR:RAW", "".join([str(expected_error_code), expected_error_message])
        )
...
class IOCResetTests(unittest.TestCase):
    ...
    def test_that_GIVEN_a_device_WHEN_reset_THEN_the_IOC_has_been_reinitalized(self):
        # Given:
        previous_number_of_times_the_device_has_been_reset = int(
            self._lewis.backdoor_run_function_on_device(

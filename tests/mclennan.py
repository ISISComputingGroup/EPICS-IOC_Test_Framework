...
class MclennanTests(unittest.TestCase):
    ...
    def test_WHEN_velocity_changes_WHEN_sending_builtin_home_THEN_creep_speed_is_changed_then_set_back_after_home(
        self,
    ):
    ...
    def test_WHEN_MOTOR_ASG_is_WASL0_THEN_prevent_HVEL_setting(self):
        ...
        with self._ioc.start_with_macros(macros, pv_to_wait_for=PV_TO_WAIT_FOR):
            with self.assertRaises(
                ...
                msg="HVEL should not be able to be set based on the default ASG level (WASL0)",
            ):

...
def _update_fields_continuously(psu_amps_at_measured_zero):
    ...
    while True:
        ...
        measured = {
            axis: (outputs[axis] - psu_amps_at_measured_zero[axis]) * (1.0 / amps_per_mg[axis])
...
class ZeroFieldTests(unittest.TestCase):
    ...
    def _set_simulated_measured_fields(self, fields, overload=False, wait_for_update=True):
        ...
        self.magnetometer_ca.set_pv_value(
            "OVERLOAD:_CALC.CALC", "1" if overload else "0", sleep_after_set=0
        )
    ...
    def _assert_status(self, status):
        ...
        if (
            name == Statuses.MAGNETOMETER_OVERLOAD[0]
            and self.zfcntrl_ca.get_pv_value("AUTOFEEDBACK") == "Manual"
        ):
    ...
    def _simulate_measured_fields_changing_with_outputs(self, psu_amps_at_measured_zero):
        ...
        thread = multiprocessing.Process(
            target=_update_fields_continuously, args=(psu_amps_at_measured_zero,)
        )
        thread.start()
        try:
            yield
    ...
    def test_WHEN_psu_sp_rbv_is_out_of_range_THEN_status_is_psu_sp_rbv_out_of_range(
        self, _, axis, rbv_output
    ):
        ca_object = axis
        fields = {"X": 1, "Y": 1, "Z": 1}
        ...
        self._set_output_limits(
            lower_limits={k: -3 for k in FIELD_AXES}, upper_limits={k: 3 for k in FIELD_AXES}
        )
    ...
    def test_GIVEN_autofeedback_WHEN_measured_field_different_from_setpoints_THEN_power_supply_outputs_move_in_correct_direction(
        ...
        adjustment_amount = (
            10 * STABILITY_TOLERANCE
        )  # To ensure that it is not considered stable to start with
        measured_fields = {
            k: measured_field_modifier(v, adjustment_amount) for k, v in fields.items()
        }
    ...
    def test_GIVEN_measured_values_updating_realistically_WHEN_in_auto_mode_THEN_converges_to_correct_answer(
        ...
        self._set_output_limits(
            lower_limits={k: -100 for k in FIELD_AXES}, upper_limits={k: 100 for k in FIELD_AXES}
        )
    ...
    def test_GIVEN_magnetometer_overloaded_THEN_error_suppressed_if_in_manual_mode(
        ...
        self.zfcntrl_ca.assert_that_pv_alarm_is(
            "STATUS",
            self.zfcntrl_ca.Alarms.MAJOR
            if overloaded and autofeedback
            else self.zfcntrl_ca.Alarms.NONE,
        )
    ...
    def test_GIVEN_ioc_is_restarted_WHEN_feedback_autosave_is_set_to_true_THEN_feedback_mode_persists(
        self, _, autofeedback
    ):
        with self._ioc.start_with_macros(
            {"SAVEFEEDBACKMODE": "YES"}, pv_to_wait_for="OUTPUT:X:VOLT:SP:RBV"
        ):
            self._set_autofeedback(autofeedback)

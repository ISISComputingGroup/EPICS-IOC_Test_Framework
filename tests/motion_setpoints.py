...
test_path = os.path.realpath(
    os.path.join(os.getenv("EPICS_KIT_ROOT"), "support", "motionSetPoints", "master", "settings")
)
...
class MotionSetpointsTests(unittest.TestCase):
    ...
    def test_GIVEN_2D_WHEN_set_position_THEN_position_is_set_and_motor_moves_to_position(self):
        for index, (
            expected_position,
            expected_motor_position_cord1,
            expected_motor_position_cord2,
        ) in enumerate(POSITIONS_2D):
    ...
    @parameterized.expand(parameterized_list([1, 2, 10]))
    def test_GIVEN_XD_WHEN_move_to_out_position_THEN_in_position_light_goes_off_then_on(
        self, _, axis_num
    ):
    ...
    @parameterized.expand(parameterized_list([1, 2, 10]))
    def test_GIVEN_XD_WHEN_set_offset_THEN_in_position_light_goes_off_then_on_and_motor_moves_to_position_plus_offset(
        self, _, axis_num
    ):
        channel_access = self.channel_access_instances[axis_num]
        offset_number = 2
    ...
    def test_GIVEN_XD_WHEN_set_large_offset_THEN_current_position_is_correct(self, _, axis_num):
        ...
        offset_coords = [
            position_table[0][coord + 1] - position_table[2][coord + 1] for coord in range(axis_num)
        ]
    ...
    def test_GIVEN_XD_WHEN_move_motor_THEN_tolerance_checked(self, _, axis_num):
        ...
        for axis in range(axis_num):
            ...
            self.motor_ca.assert_that_pv_is_number(
                motor_pv + ".RBV", position_to_test[axis + 1] - 0.2, tolerance=0.00001
            )
        channel_access.assert_that_pv_is("POSN", position_to_test[0])
        channel_access.assert_that_pv_is("IPOSN", 1)
        channel_access.assert_that_pv_is("POSITIONED", 1)
    ...
    @parameterized.expand(parameterized_list([3, 6, 9]))
    ...
    def test_WHEN_lookup_modified_and_reset_called_THEN_new_position_names_picked_up(self):
        ...
        modify_file_and_assert_test(
            self.ca10D, lookup_file, "Sample1", new_position_name, test_position_updated
        )

    def test_WHEN_lookup_modified_to_be_in_error_and_reset_called_THEN_new_position_names_picked_up(
        self,
    ):
        lookup_file = os.path.join(test_path, "lookup10D.txt")
    ...
    def test_GIVEN_in_position_WHEN_lookup_modified_and_reset_called_THEN_no_longer_in_position(
        self,
    ):
        ...
        modify_file_and_assert_test(
            self.ca2D, lookup_file, MOTOR_POSITION_SAMPLE1_COORD1, new_coord, test_not_in_position
        )

    def test_GIVEN_in_position_WHEN_lookup_modified_and_move_to_updated_position_THEN_go_to_new_position(
        self,
    ):
        lookup_file = os.path.join(test_path, "lookup2D.txt")
        new_coord = 7
    ...
    def test_GIVEN_files_WHEN_file_names_checked_THEN_current_names_correct(self):
        ...
        lookup_path = posixpath.join(
            os.getenv("EPICS_KIT_ROOT").replace(os.sep, "/"),
            "support",
            "motionSetPoints",
            "master",
            "settings",
        ...
        self.caDN.assert_that_pv_is(
            "FILENAME", str(posixpath.join(lookup_path, "duplicate_names.txt"))
        )
        self.caDP.assert_that_pv_is(
            "FILENAME", str(posixpath.join(lookup_path, "duplicate_positions.txt"))
        )

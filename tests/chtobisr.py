...
class ChtobisrTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.ca = {
            1: ChannelAccess(device_prefix=DEVICE_PREFIX + ":1", default_timeout=30),
            2: ChannelAccess(device_prefix=DEVICE_PREFIX + ":2", default_timeout=30),
            3: ChannelAccess(device_prefix=DEVICE_PREFIX + ":3", default_timeout=30),
        }
    ...
    @skip_if_recsim("Lewis backdoor not available in RecSim")
    def test_GIVEN_status_active_WHEN_read_status_THEN_pv_reads_status_state(
        self, _, backdoor_name, pv_name
    ):
    ...
    @skip_if_recsim("Lewis backdoor not available in RecSim")
    def test_GIVEN_fault_active_WHEN_read_faults_THEN_pv_reads_fault_state(
        self, _, backdoor_name, pv_name
    ):
        for state in [False, True]:
            self._lewis.backdoor_run_function_on_device(

...
def modified_environment(**kwargs: str) -> Generator[None, None, None]:
    ...
    """
    # Copying old values
    old_env = {name: os.environ.get(name, "") for name in kwargs}
...
class ModuleTests:
    ...
    def __get_file_reference(self) -> ModuleType:
        module = load_module(f"tests.{self.__name}")
        return module
...
def check_test_modes(module: ModuleType) -> set[TestModes]:
    ...
    try:
        ...
        raise ValueError(
            f"Expected test module {module.__name__} to contain a TEST_MODES attribute"
        )
...
def check_build_architectures(module: ModuleType) -> set[BuildArchitectures]:
    ...
    try:
        ...
        architectures = {BuildArchitectures._64BIT, BuildArchitectures._32BIT}

...
class RikenChangeover(object, metaclass=ABCMeta):
    ...
    def test_GIVEN_value_on_input_ioc_changes_THEN_coord_psus_disable_pv_updates_with_the_same_value(
        self,
    ):
    ...
    def test_GIVEN_all_power_supplies_off_WHEN_value_on_input_ioc_changes_THEN_power_supplies_have_their_disp_field_set(
        self,
    ):
    ...
    def test_GIVEN_plc_cancels_changeover_before_psus_are_all_switched_off_WHEN_psus_become_switched_off_THEN_they_do_not_get_disabled(
        self,
    ):
        self._set_all_power_supply_states(True)
        self._set_input_pv(False)
        self._assert_all_power_supplies_disabled(
            False
        )  # Power supplies not disabled because still powered on
    ...
    def test_GIVEN_changeover_initiated_WHEN_power_supplies_off_THEN_acknowledgement_pv_true(self):
        ...
        self._set_input_pv(
            True
        )  # Some time later the PLC sends signal to say it has finished the changeover sequence
        self.ca.assert_that_pv_is_number(self.get_acknowledgement_pv(), 0)

    def test_GIVEN_changeover_sequence_completes_THEN_power_supplies_are_reenabled_after_sequence(
        self,
    ):
        self._set_all_power_supply_states(True)
        self._set_input_pv(False)
        ...
        self._set_all_power_supply_states(
            False
        )  # Power supplies now switched off so changeover can continue
        ...
        self.ca.assert_that_pv_is(
            self.get_acknowledgement_pv(), 0
        )  # And "ok to run changeover" line should be cleared
